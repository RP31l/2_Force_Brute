...
if "bloc_actuel" not in st.session_state:
    st.session_state.bloc_actuel = debut
...
with col1:
    if st.button("🚀 ANALYSER CE BLOC"):
        ...
        if not msg:
            ...
            with st.spinner(f"Test des clés {st.session_state.bloc_actuel:,} à {st.session_state.bloc_actuel + TAILLE_BLOC - 1:,}..."):
                top = analyser(st.session_state.bloc_actuel, st.session_state.bloc_actuel + TAILLE_BLOC, msg, nb)
...
with col2:
    if st.button("⏭️ BLOC SUIVANT"):
        st.session_state.bloc_actuel = fin
        if not msg:
            st.error("Entrez un message chiffré !")
        else:
            with st.spinner(f"Test des clés {fin:,} à {fin + TAILLE_BLOC - 1:,}..."):
                top = analyser(fin, fin + TAILLE_BLOC, msg, nb)
            nouveau_bloc = fin // TAILLE_BLOC
            st.markdown(f"<p style='color:#8a9ab0;font-size:0.85rem;'>✅ Bloc {nouveau_bloc} testé</p>", unsafe_allow_html=True)
            if top:
                st.markdown("<p style='color:#00ff88;font-weight:700;letter-spacing:0.2em;'>🏆 MEILLEURS RÉSULTATS :</p>", unsafe_allow_html=True)
                for i, (score, cle, res) in enumerate(top):
                    if i == 0:
                ...
                st.warning(f"Rien trouvé. Prochain bloc : {fin + TAILLE_BLOC:08d}")
